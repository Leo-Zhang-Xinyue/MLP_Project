...
class Run_Iterations(object):
    def __init__(self, model, x_train, x_train_char, y_train, index2word, batch_size, num_iters,
                 learning_rate, tracking_pair=False, x_val=[], x_val_char=[], y_val=[], print_every=1, plot_every=1):
        ...
        self.x_train_char = x_train_char
        self.y_train = y_train
        self.train_samples = len(self.x_train)

        # Development data.
        self.x_val = x_val
        self.x_val_char = x_val_char
        self.y_val = y_val
    ...
    def train_iters(self):
        ...
        for epoch in range(1, self.num_iters + 1):
            for i in range(0, self.train_samples, self.batch_size):
                ...
                #print(self.x_train)
                #print('@@@@@@@@@x_train_char')
                #print(self.x_train_char)
                #print(self.x_train_char)
                
                input_char = self.x_train_char[i : i + self.batch_size]
                #print('*************')
                #print(input_char)
                ...
                #print("@@@@@after batch input_char")
                #print(input_char)
                #print('@@@@@@@@after batch x_train')
                #print(input_variables)
                #######################################################
                #这个loss 要传入 input——char
                loss, _ = self.model.train(input_variables, input_char, similarity_scores, self.criterion, model_optimizer)
    ...
    def evaluate(self, seqs,chars, scores):
        loss, similarity_scores = self.model.train(seqs, chars, scores, self.criterion, evaluate=True)
        return loss, similarity_scores

    def evaluate_specific(self, seqs, chars, score, name='tracking_pair'):
        ...
        _, similarity_score = self.evaluate([seqs], [chars], score)
        print('<', similarity_score.item())

    def evaluate_randomly(self, n=10):
        for i in range(n):
            ind = random.randrange(self.val_samples)
            self.evaluate_specific(self.x_val[ind], self.x_val_char[ind], self.y_val[ind], name=str(i))
    ...
    def get_accuracy(self):
        ...
        for i in range(0, self.val_samples, self.batch_size):
            ...
            actual_scores = self.y_val[i : i + self.batch_size] 
            input_char = self.x_val_char[i : i + self.batch_size]
            # Batch Size
         #input_char = self.self.x_train_char[i : i + self.batch_size] 
            ##############################################################
            #val 这还有很多要处理的
            
            loss, predicted_scores = self.model.train(input_variables,input_char, actual_scores, self.criterion, evaluate=True)
            total_loss += loss
